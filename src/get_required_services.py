...
def get_ffr(T_MAX_FFR=10, n_points=1000):
    ...
    require_ffr_max = {(0, .7): (MP_max, MP_max), (.7, 5): (MP_max, MP_max), (5, 10): (MP_max, 0)}
...
def get_fcr(T_MAX_FCR=60, n_points=1000):
    ...
    require_fcr_max = {(0, ta): (RP_MAX, RP_MAX), (ta, 60): (RP_MAX, RP_MAX)}
...
def get_fcr_d(T_MAX=60, n_points=1000):
    D1 = .86  # at 7.5s
    DMAX = D1  # maximum overshoot is 20% so reference is not allowed to be above 1.2*D1
    require_fcr_d = {(0.05, 7.5): (0, D1), (7.5, T_MAX): (D1, D1)}
    require_fcr_d_max = {(0, 7.5): (DMAX, DMAX), (7.5, T_MAX): (DMAX, DMAX)}
